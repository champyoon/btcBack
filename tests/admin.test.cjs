...
window.updates = 0; window.privateReads = 0;
window.records = Array.from({length: 27}, (_,i) => ({
  id: String(i), created_at: new Date(Date.UTC(2026,0,28-i)).toISOString(),
  store_name: 'Test Store', purchase_date: '2026-01-01', purchase_amount: 52000,
...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(()=>window.records[2].status==='paid');
    assert.ok(paidWarning.includes('실제로 Bitcoin 지급을 완료했는지 확인하세요'));
    ...
    for(const width of [1920,768,390,320]) {
      ...
      if(width===1920||width===390) await page.screenshot({path:path.join(os.tmpdir(),'btcback-admin-'+width+'.png'),fullPage:true});
    }
    await page.locator('#requests tr').first().getByRole('button',{name:'상세',exact:true}).click();
