...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#panel-coupang a:not(#coupang-anchor-link), #panel-coupang img, #panel-coupang iframe, .standalone-banner').count(), 0);
    assert.equal(await page.locator('#coupang-anchor-link').getAttribute('href'), null);
    assert.equal(await page.locator('#panel-coupang button').count(), 2);
    assert.ok((await page.locator('#panel-coupang .disclosure').textContent()).includes('이에 따른 일정액의 수수료를 제공받습니다.'));
