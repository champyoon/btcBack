...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#panel-coupang a, #panel-coupang img, #panel-coupang iframe, .standalone-banner').count(), 0);
    assert.equal(await page.locator('#panel-coupang button').count(), 1);
