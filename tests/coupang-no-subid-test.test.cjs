...
assert(!/landingUrl|tracking_id/.test(source));
(async () => {
  ...
  try {
    ...
    await context.route('**/*', async route => {
      ...
      if (url.hostname === 'btcback.test' && ['/shopping.html','/common.css','/coupang-no-subid-test.js'].includes(url.pathname)) {
    ...
    assert.equal(await prepare.innerText(), '쿠팡으로 이동');
    ...
    assert((await page.locator('#coupang-no-subid-status').innerText()).length > 0);
    failure = false;
